...
export default function WizardPage() {
  ...
  // Load this wizard's state from localStorage on mount
  useEffect(() => {
    if (!wizardId) return;
  ...
  function goNext() {
    ...
    if (state.currentStep < WIZARD_STEPS.length) {
      ...
      // Wizard complete — create a draft and navigate to the editor
  ...
  if (!loaded) {
    return <div className="max-w-2xl mx-auto py-16 text-center text-slate-400">Loading…</div>;
  }
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5 font-mono">#{wizardId.slice(0, 8)}</p>
        </div>
